...
> What "done" means in [PROJECT_NAME]. If a line can't be checked, the change
> isn't done — it's _claimed_ done.
> **Last updated**: [DATE]

## Behavior

- [ ] The full flow was exercised end-to-end (by hand or with a high-level
      automated test), not just unit-tested.
- [ ] The happy path **and at least two failure paths** (invalid input, a
      dependency failing) respond sensibly.
- [ ] Loading, empty, error and success states render correctly (if there is a UI).
...
- [ ] Coverage of the changed lines is meaningful (see [`testing.md`](testing.md)).

## Interfaces and data

- [ ] Public API changes are documented in
      [`../architecture/api.md`](../architecture/api.md); breaking changes have a
      migration note in `CHANGELOG.md`.
- [ ] Error messages are actionable, not "something went wrong".
- [ ] Migrations are reversible and safe to run hot (see [`database.md`](database.md)).
- [ ] No secrets in code or logs (see [`secrets.md`](secrets.md)).
